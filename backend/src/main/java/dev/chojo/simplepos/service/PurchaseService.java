...
public class PurchaseService {
    ...
    public PurchaseService(StorageService storageService, ProductRepository productRepository, PurchaseRepository purchaseRepository) {
                ...
                for (RecipeEntry recipe : prod.getRecipe().getEntries()) {
                    ...
                    for (StorageTransaction transaction : transactions) {
                        for (int i = 0; i < transaction.getAmount(); i++) {
                            parts.add(new PurchasePart(null, purchase, transaction.getStorage()));
                        }
                    }
                }
